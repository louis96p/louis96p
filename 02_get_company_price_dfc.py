...
Stocklist = pd.DataFrame(Stocklist_read, columns =["symbol"]).head(7500)
...
for i in range(0, len(ListTotal), max_batch):
    ...
    L_CompanyInfoList = [] 
    for Code in List:
     url =  ("https://financialmodelingprep.com/api/v3/historical-discounted-cash-flow-statement/" + Code + "?period=quarter&" +APIKEY)
     L_CompanyInfo = pd.read_json(url)#.sort_values(by='date', ascending=True)
     #L_CompanyInfo['pricechange']= L_CompanyInfo['price'].pct_change().fillna(0)
     #L_CompanyInfo['dcf']= L_CompanyInfo['dcf'].pct_change().fillna(0)
...
F_CompanyInfo.to_csv (r"C:\\Users\\DELPILL2\\OneDrive - EY\\Documents\\Privat\\Python\\Sample\\StockKpi\\companystock_dfc_2.csv", index = None)
print (F_CompanyInfo)
